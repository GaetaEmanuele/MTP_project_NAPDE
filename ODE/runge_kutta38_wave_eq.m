function [U,V]=runge_kutta38_wave_eq(Dati,x,dependence,Matrix,list_U ,list_V,front,t_hat)
    ...
    for n = 1:num_steps-1
        ...
        k2 = rk4_step(Yi + dt * k1/3, ti + (1/3)* dt,x,Dati,Matrix,front);

        k3 = rk4_step(Yi -(1/3) * dt * k1 + k2, ti + (2/3) * dt,x,Dati,Matrix,front);

        k4 = rk4_step(Yi +k1-k2+k3, ti + dt,x,Dati,Matrix,front);

        
        % update the solution
        Y(:,n+1) = Yi + (dt / 8) * (k1 + 3*k2 + 3*k3 + k4);
        
        Y(1,n+1) = u0(1);
        Y(length(u0),n+1) = u0(end);
        Y(length(u0)+1,n+1) = v0(1);
        Y(end,n+1) = v0(end);
